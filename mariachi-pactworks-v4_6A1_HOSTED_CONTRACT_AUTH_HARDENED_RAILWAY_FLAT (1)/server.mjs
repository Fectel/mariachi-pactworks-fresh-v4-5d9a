...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/hosted-contracts'){
    ...
    // Revoke prior active version for same contract/version before creating replacement.
    try{await sb(`hosted_contracts?org_id=eq.${encodeURIComponent(ORG_ID)}&contract_external_id=eq.${encodeURIComponent(b.contractExternalId)}&contract_version=eq.${version}`,{method:'PATCH',payload:{status:'revoked'}})}catch{}
    const rows=await sb('hosted_contracts',{method:'POST',payload:{
      org_id:ORG_ID,contract_external_id:String(b.contractExternalId),contract_version:version,
    ...
    await insertEvent(row.id,'hosted_link_created','admin',{contractExternalId:b.contractExternalId},u.id);
  ...
  if(req.method==='POST'&&declare){
    ...
    const choice=b.choice==='full'?'full':'deposit',amount=choice==='full'?total:deposit;
